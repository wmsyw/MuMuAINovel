...
async def create_chapter(
    chapter: ChapterCreate, request: Request, db: AsyncSession = Depends(get_db)
):
    """创建新的章节"""
    # 验证用户权限和项目是否存在
    ...
    db_chapter = Chapter(**chapter.model_dump(), word_count=word_count)
    db.add(db_chapter)
...
async def get_chapter(
    chapter_id: str, request: Request, db: AsyncSession = Depends(get_db)
):
    """根据ID获取章节详情"""
    ...
    if "content" in update_data:
        ...
        if project:
            project.current_words = (
                project.current_words - old_word_count + new_word_count
            )
            ...
            if not chapter.content or chapter.content.strip() == "":
                ...
                try:
                    ...
                    if foreshadow_result["deleted_count"] > 0:
                        logger.info(
                            f"🔮 已清理章节 {chapter_id[:8]} 的 {foreshadow_result['deleted_count']} 个伏笔数据"
                        )
                except Exception as e:
                    logger.warning(f"⚠️ 清理伏笔数据失败: {str(e)}")

                logger.info(
                    f"🗑️ 章节 {chapter_id[:8]} 内容已清空，已清理分析、记忆和伏笔数据"
                )
...
async def delete_chapter(
    ...
    if project:
        ...
        project.current_words = max(
            0, (project.current_words or 0) - chapter_word_count
        )

    # 🗑️ 清理向量数据库中的记忆数据
    try:
        await memory_service.delete_chapter_memories(
            user_id=user_id, project_id=chapter.project_id, chapter_id=chapter_id
        )
        logger.info(f"✅ 已清理章节 {chapter_id[:8]} 的向量记忆数据")
    except Exception as e:
        logger.warning(f"⚠️ 清理向量记忆数据失败: {str(e)}")
...
async def check_prerequisites(
    ...
    # 检查是否所有前置章节都有内容
    incomplete_chapters = [
        ch for ch in previous_chapters if not ch.content or ch.content.strip() == ""
    ]
...
async def build_characters_info_with_careers(
    ...
    if filter_character_names:
        filtered_characters = [
            c for c in characters if c.name in filter_character_names
        ]
        if not filtered_characters:
            logger.warning(
                f"筛选后无匹配角色，使用全部角色。筛选名单: {filter_character_names}"
            )
            filtered_characters = characters
        else:
            logger.info(
                f"根据筛选名单保留 {len(filtered_characters)}/{len(characters)} 个角色: {[c.name for c in filtered_characters]}"
            )
    ...
    for c in characters:
        ...
        if c.current_state:
            state_preview = (
                c.current_state[:50] if len(c.current_state) > 50 else c.current_state
            )
...
async def analyze_chapter_background(
    chapter_id: str, user_id: str, project_id: str, task_id: str, ai_service: AIService
) -> bool:
    """
    ...
    try:
        ...
        logger.info(
            f"📋 后台分析 - 已获取{len(existing_foreshadows)}个已埋入伏笔用于匹配（含智能回收标记）"
        )
        ...
        if chapter.expansion_plan:
            try:
                ...
                if focus_names:
                    ...
                    logger.info(
                        f"📋 从expansion_plan提取角色焦点: {filter_character_names}"
                    )
            ...
            try:
                ...
                if chapter_outline and chapter_outline.structure:
                    ...
                    if raw_characters:
                        ...
                        logger.info(
                            f"📋 从outline.structure提取角色: {filter_character_names}"
                        )
            except (json.JSONDecodeError, Exception):
                pass
        ...
        if filter_character_names:
            characters_query = characters_query.where(
                Character.name.in_(filter_character_names)
            )
        ...
        async def on_retry_callback(
            attempt: int, max_retries: int, wait_time: int, error_reason: str
        ):
            ...
            try:
                async with write_lock:
                    ...
                    if task_retry:
                        ...
                        task_retry.started_at = (
                            datetime.now()
                        )  # 重置开始时间，防止超时检测误判
                        task_retry.progress = 25 + attempt * 5  # 根据重试次数更新进度
                        task_retry.error_message = (
                            f"正在重试({attempt}/{max_retries})：{error_reason[:100]}"
                        )
                        await db_session.commit()
                        logger.info(
                            f"🔄 分析任务重试状态已更新: 尝试 {attempt}/{max_retries}, 等待 {wait_time}s, 原因: {error_reason[:50]}..."
                        )
        ...
        async with write_lock:
            ...
            if existing_analysis:
                ...
                existing_analysis.conflict_level = analysis_result.get(
                    "conflict", {}
                ).get("level", 0)
                existing_analysis.conflict_types = analysis_result.get(
                    "conflict", {}
                ).get("types", [])
                existing_analysis.emotional_tone = analysis_result.get(
                    "emotional_arc", {}
                ).get("primary_emotion", "")
                existing_analysis.emotional_intensity = (
                    analysis_result.get("emotional_arc", {}).get("intensity", 0) / 10.0
                )
                ...
                existing_analysis.foreshadows_planted = sum(
                    1
                    for f in analysis_result.get("foreshadows", [])
                    if f.get("type") == "planted"
                )
                existing_analysis.foreshadows_resolved = sum(
                    1
                    for f in analysis_result.get("foreshadows", [])
                    if f.get("type") == "resolved"
                )
                existing_analysis.plot_points = analysis_result.get("plot_points", [])
                existing_analysis.plot_points_count = len(
                    analysis_result.get("plot_points", [])
                )
                existing_analysis.character_states = analysis_result.get(
                    "character_states", []
                )
                existing_analysis.scenes = analysis_result.get("scenes", [])
                existing_analysis.pacing = analysis_result.get("pacing", "moderate")
                existing_analysis.overall_quality_score = analysis_result.get(
                    "scores", {}
                ).get("overall", 0)
                existing_analysis.pacing_score = analysis_result.get("scores", {}).get(
                    "pacing", 0
                )
                existing_analysis.engagement_score = analysis_result.get(
                    "scores", {}
                ).get("engagement", 0)
                existing_analysis.coherence_score = analysis_result.get(
                    "scores", {}
                ).get("coherence", 0)
                existing_analysis.analysis_report = analyzer.generate_analysis_summary(
                    analysis_result
                )
                existing_analysis.suggestions = analysis_result.get("suggestions", [])
                existing_analysis.dialogue_ratio = analysis_result.get(
                    "dialogue_ratio", 0
                )
                existing_analysis.description_ratio = analysis_result.get(
                    "description_ratio", 0
                )
            else:
                # 创建新记录
                ...
                plot_analysis = PlotAnalysis(
                    ...
                    emotional_tone=analysis_result.get("emotional_arc", {}).get(
                        "primary_emotion", ""
                    ),
                    emotional_intensity=analysis_result.get("emotional_arc", {}).get(
                        "intensity", 0
                    )
                    / 10.0,
                    ...
                    foreshadows_planted=sum(
                        1
                        for f in analysis_result.get("foreshadows", [])
                        if f.get("type") == "planted"
                    ),
                    foreshadows_resolved=sum(
                        1
                        for f in analysis_result.get("foreshadows", [])
                        if f.get("type") == "resolved"
                    ),
                    ...
                    overall_quality_score=analysis_result.get("scores", {}).get(
                        "overall", 0
                    ),
                    pacing_score=analysis_result.get("scores", {}).get("pacing", 0),
                    engagement_score=analysis_result.get("scores", {}).get(
                        "engagement", 0
                    ),
                    coherence_score=analysis_result.get("scores", {}).get(
                        "coherence", 0
                    ),
            ...
            async with write_lock:
                clean_result = (
                    await foreshadow_service.clean_chapter_analysis_foreshadows(
                        db=db_session, project_id=project_id, chapter_id=chapter_id
                    )
                )
            if clean_result["cleaned_count"] > 0:
                logger.info(
                    f"🧹 重新分析前清理了 {clean_result['cleaned_count']} 个旧伏笔"
                )
        ...
        async with write_lock:
            for mem in memories:
                ...
                if text_position >= 0:
                    logger.debug(
                        f"  保存记忆 {memory_id}: position={text_position}, length={text_length}"
                    )
        ...
        if memory_records:
            added_count = await memory_service.batch_add_memories(
                user_id=user_id, project_id=project_id, memories=memory_records
            )
        ...
        if analysis_result.get("organization_states"):
            try:
                ...
                async with write_lock:
                    org_state_result = (
                        await CharacterStateUpdateService.update_organization_states(
                            db=db_session,
                            project_id=project_id,
                            organization_states=analysis_result.get(
                                "organization_states", []
                            ),
                            chapter_number=chapter.chapter_number,
                        )
                ...
                # 组织状态更新失败不应影响整个分析流程
                logger.error(
                    f"⚠️ 更新组织自身状态失败: {str(org_state_error)}", exc_info=True
                )
        ...
        if analysis_result.get("foreshadows"):
            try:
                ...
                async with write_lock:
                    foreshadow_stats = (
                        await foreshadow_service.auto_update_from_analysis(
                            db=db_session,
                            project_id=project_id,
                            chapter_id=chapter_id,
                            chapter_number=chapter.chapter_number,
                ...
                if (
                    foreshadow_stats["planted_count"] > 0
                    or foreshadow_stats["resolved_count"] > 0
                ):
                    logger.info(
                ...
                # 伏笔更新失败不应影响整个分析流程
                logger.error(
                    f"⚠️ 自动更新伏笔失败: {str(foreshadow_error)}", exc_info=True
                )
        else:
        ...
        for retry in range(3):
            try:
                async with write_lock:
                    ...
                    update_success = True
                    logger.info(
                        f"✅ 章节分析完成: {chapter_id}, 提取{len(memories)}条记忆"
                    )
                    break
            except Exception as commit_error:
                logger.error(
                    f"❌ 提交任务完成状态失败(重试{retry + 1}/3): {str(commit_error)}"
                )
                if retry < 2:
                ...
                try:
                    async with write_lock:
                        ...
                        if task:
                            ...
                            await db_session.commit()
                            logger.info(
                                f"✅ 任务状态已更新为failed: {task_id} (重试{retry + 1}次)"
                            )
                            break
                        else:
                    ...
                    logger.error(
                        f"❌ 更新任务状态失败(重试{retry + 1}/3): {str(update_error)}"
                    )
                    if retry < 2:
                        await asyncio.sleep(0.1)  # 短暂等待后重试
                    else:
                        logger.error(
                            f"❌ 任务状态更新失败，已达到最大重试次数: {task_id}"
                        )
...
async def generate_chapter_content_stream(
    ...
    custom_model = (
        generate_request.model if hasattr(generate_request, "model") else None
    )
    ...
    async for temp_db in get_db(request):
        try:
            ...
            # 检查前置条件
            can_generate, error_msg, previous_chapters = await check_prerequisites(
                temp_db, chapter
            )
            if not can_generate:
        ...
        try:
            ...
            async for db_session in get_db(request):
                ...
                if style_id:
                    ...
                    if style:
                        ...
                        if style.user_id is None or style.user_id == current_user_id:
                            ...
                            style_type = (
                                "全局预设" if style.user_id is None else "用户自定义"
                            )
                ...
                if outline_mode == "one-to-one":
                    ...
                    logger.info(
                        f"  - 大纲长度: {chapter_context.context_stats.get('outline_length', 0)} 字符"
                    )
                    logger.info(
                        f"  - 上一章内容: {chapter_context.context_stats.get('previous_content_length', 0)} 字符"
                    )
                    logger.info(
                    ...
                    logger.info(
                        f"  - 总长度: {chapter_context.context_stats.get('total_length', 0)} 字符"
                    )
                else:
                    # ========== 1-N模式：使用独立的完整构建器 ==========
                    ...
                    logger.info(f"  - 章节序号: {current_chapter.chapter_number}")
                    logger.info(
                        f"  - 衔接锚点: {chapter_context.context_stats.get('continuation_length', 0)} 字符"
                    )
                    logger.info(
                        f"  - 角色信息: {chapter_context.context_stats.get('characters_length', 0)} 字符"
                    )
                    ...
                    logger.info(
                        f"  - 故事骨架: {chapter_context.context_stats.get('skeleton_length', 0)} 字符"
                    )
                    logger.info(
                        f"  - 伏笔提醒: {chapter_context.context_stats.get('foreshadow_length', 0)} 字符"
                    )
                ...
                if outline_mode == "one-to-one":
                    ...
                    if chapter_context.continuation_point:
                        ...
                        base_prompt = PromptService.format_prompt(
                            ...
                            previous_chapter_summary=chapter_context.previous_chapter_summary
                            or "（无上一章摘要）",
                            ...
                            or "暂无需要关注的伏笔",
                            relevant_memories=chapter_context.relevant_memories
                            or "暂无相关记忆",
                        )
                        logger.debug(
                            f"创建第{current_chapter.chapter_number}章提示词: {base_prompt}"
                        )
                    else:
                        # 第一章
                        template = await PromptService.get_template(
                            "CHAPTER_GENERATION_ONE_TO_ONE", current_user_id, db_session
                        )
                        base_prompt = PromptService.format_prompt(
                            template,
                            project_title=project.title,
                    ...
                    if chapter_context.continuation_point:
                        ...
                        logger.info(
                            f"📝 [1-n模式] 使用带上下文的模板（第{current_chapter.chapter_number}章）"
                        )
                        ...
                        base_prompt = PromptService.format_prompt(
                            ...
                            recent_chapters_context=chapter_context.recent_chapters_context
                            or "",
...
确保在整个章节创作过程中始终保持风格的一致性。"""
                    logger.info(
                        f"✅ 已将写作风格注入系统提示词（{len(style_content)}字符）"
                    )
                ...
                calculated_max_tokens = max(
                    2000, min(calculated_max_tokens, 16000)
                )  # 限制在 2000-16000 之间
                logger.info(
                ...
                yield await tracker.generating(
                    current_chars=0, estimated_total=target_word_count
                )
                ...
                try:
                    plant_result = (
                        await foreshadow_service.auto_plant_pending_foreshadows(
                            db=db_session,
                            project_id=project.id,
                            chapter_id=chapter_id,
                            chapter_number=current_chapter.chapter_number,
                    ...
                    if plant_result.get("planted_count", 0) > 0:
                        logger.info(
                            f"🔮 自动标记伏笔已埋入: {plant_result['planted_count']}个"
                        )
                ...
                yield await tracker.result(
                    {"word_count": new_word_count, "analysis_task_id": task_id}
                )

                # 发送分析开始事件（使用自定义事件）
                yield await SSEResponse.send_event(
                    event="analysis_started",
                    data={"task_id": task_id, "message": "章节分析已开始"},
                )
...
def _build_analysis_task_status_payload(
    chapter_id: str, task: Optional[AnalysisTask], auto_recovered: bool = False
) -> dict:
    """统一构建分析任务状态响应"""
    if not task:
        return {
...
async def get_analysis_task_status(
    ...
    if task.status == "running":
        ...
        if task.started_at and (current_time - task.started_at) > timedelta(
            minutes=timeout_minutes
        ):
            task.status = "failed"
            task.error_message = (
                f"任务超时（超过{timeout_minutes}分钟未完成，已自动恢复）"
            )
...
@router.post(
    "/project/{project_id}/analysis/statuses",
    summary="批量查询章节分析任务状态",
...
async def get_project_analysis_task_statuses(
    ...
    return {"project_id": project_id, "total": len(chapter_ids), "items": items}
...
async def _run_batch_analysis_in_sequence(
    ...
    for index, task_item in enumerate(tasks_queue, start=1):
        ...
        logger.info(
            f"🔁 一键分析顺序执行中 [{index}/{len(tasks_queue)}]：第{chapter_number}章"
        )
        try:
            ...
            if not success:
                logger.warning(
                    f"⚠️ 一键顺序分析返回失败: chapter_id={chapter_id}, task_id={task_id}"
                )
        except Exception as e:
    ...
    if tasks_to_start:
        try:
            ...
            for chapter, analysis_task in tasks_to_start:
                started_tasks[chapter.id] = _build_analysis_task_status_payload(
                    chapter.id, analysis_task
                )
            ...
            raise HTTPException(
                status_code=500, detail=f"一键分析创建任务失败: {str(e)}"
            )
...
async def get_chapter_annotations(
    ...
    for mem in memories:
        ...
        length = (
            mem.text_length
            if hasattr(mem, "text_length") and mem.text_length is not None
            else 0
        )
        metadata_extra = {}
                    ...
                    if foreshadow.get("content") in mem.content:
                        ...
                        if keyword:
                            ...
                            if pos != -1:
                                position = pos
                                length = len(keyword)
                        metadata_extra["foreshadow_type"] = foreshadow.get(
                            "type", "planted"
                        )
    ...
    return {
        ...
        "summary": {
            ...
            "character_events": len(
                [a for a in annotations if a["type"] == "character_event"]
            ),
        },
    }
...
def calculate_estimated_time(
    chapter_count: int, target_word_count: int, enable_analysis: bool
) -> int:
    """
    计算预估耗时（分钟）
    ...
    total_time = chapter_count * (
        generation_time_per_chapter + analysis_time_per_chapter
    )
...
async def batch_generate_chapters_in_order(
    ...
    chapters_to_generate = [
        ch for ch in all_chapters if start_number <= ch.chapter_number <= end_number
    ]
...
async def cancel_batch_generation(batch_id: str, db: AsyncSession = Depends(get_db)):
    ...
    if task.status in ["completed", "failed", "cancelled"]:
        raise HTTPException(
            status_code=400, detail=f"任务已处于 {task.status} 状态，无法取消"
        )
...
async def execute_batch_generation_in_order(
    ...
    try:
        ...
        for idx, chapter_id in enumerate(task.chapter_ids, 1):
            ...
            while retry_count <= task.max_retries and not chapter_success:
                try:
                    ...
                    if retry_count > 0:
                        logger.info(
                            f"🔄 [{idx}/{task.total_chapters}] 重试生成章节 (第{retry_count}次): 第{chapter.chapter_number}章 《{chapter.title}》"
                        )
                    else:
                        logger.info(
                            f"📝 [{idx}/{task.total_chapters}] 开始生成章节: 第{chapter.chapter_number}章 《{chapter.title}》"
                        )
                    ...
                    if generated_summary:
                        ...
                        logger.info(
                            f"📝 已更新上一章摘要上下文: {last_generated_summary[:50]}..."
                        )
                    ...
                    if task.enable_analysis:
                        logger.info(
                            f"🔍 开始同步分析章节: 第{chapter.chapter_number}章"
                        )
                        ...
                        while analysis_retry_count < 3 and not analysis_success:
                            try:
                                if analysis_retry_count > 0:
                                    logger.info(
                                        f"🔄 重试分析章节 (第{analysis_retry_count}次): 第{chapter.chapter_number}章"
                                    )
                                ...
                                if not analysis_result:
                                    ...
                                    logger.error(
                                        f"❌ 章节分析失败: 第{chapter.chapter_number}章"
                                    )
                                ...
                                logger.info(
                                    f"✅ 章节分析成功: 第{chapter.chapter_number}章"
                                )
                                ...
                                if analysis_retry_count < 3:
                                    ...
                                    logger.warning(
                                        f"⏳ 分析失败，等待 {wait_time} 秒后重试..."
                                    )
                                    await asyncio.sleep(wait_time)
                                else:
                                    # 达到最大重试次数，必须终止整个批量任务
                                    logger.error(
                                        f"❌ 章节分析失败，已达最大重试次数(3次): 第{chapter.chapter_number}章"
                                    )
                                    ...
                                    async with write_lock:
                                        ...
                                        # 标记任务失败并终止
                                        task.status = "failed"
                                        task.error_message = f"第{chapter.chapter_number}章分析失败(重试3次): {last_analysis_error}"[
                                            :500
                                        ]
                                        task.completed_at = datetime.now()
                                    ...
                                    logger.error(
                                        f"🛑 批量生成中断: 第{chapter.chapter_number}章分析失败"
                                    )
                                    return  # 立即终止整个批量生成任务
                    ...
                    logger.info(
                        f"✅ 进度: {task.completed_chapters}/{task.total_chapters}"
                    )
                    ...
                    if retry_count <= task.max_retries:
                        ...
                        # 达到最大重试次数，记录失败信息
                        logger.error(
                            f"❌ 章节生成失败，已达最大重试次数({task.max_retries}): 第{chapter.chapter_number if chapter else '?'}章"
                        )
                        ...
                        async with write_lock:
                            ...
                            # 标记任务失败并终止
                            task.status = "failed"
                            task.error_message = f"第{chapter.chapter_number}章生成失败(重试{retry_count - 1}次): {last_error}"[
                                :500
                            ]
                            task.completed_at = datetime.now()
                            task.current_retry_count = 0
                        ...
                        if task.enable_analysis:
                            logger.error(
                                f"🛑 批量生成中断: 因启用同步分析，任何错误都会中断任务以确保职业信息和剧情连贯性"
                            )
                        else:
                            logger.error(
                                f"🛑 批量生成终止于第{chapter.chapter_number}章"
                            )
        ...
        logger.info(
            f"✅ 批量生成任务全部完成: {batch_id}, 成功生成 {task.completed_chapters} 章"
        )
...
async def generate_single_chapter_for_batch(
    ...
    if outline_mode == "one-to-one":
        ...
        context_builder = OneToOneContextBuilder(
            memory_service=memory_service, foreshadow_service=foreshadow_service
        )
        chapter_context = await context_builder.build(
            chapter=chapter,
            project=project,
    ...
    logger.info(
        f"  - 衔接锚点长度: {len(chapter_context.continuation_point or '')} 字符"
    )
    logger.info(
    ...
    if outline_mode == "one-to-one":
        ...
        if chapter_context.continuation_point:
            # 有上一章内容
            template = await PromptService.get_template(
                "CHAPTER_GENERATION_ONE_TO_ONE_NEXT", user_id, db_session
            )
            base_prompt = PromptService.format_prompt(
                template,
                project_title=project.title,
            ...
            # 第一章
            template = await PromptService.get_template(
                "CHAPTER_GENERATION_ONE_TO_ONE", user_id, db_session
            )
            base_prompt = PromptService.format_prompt(
                template,
                project_title=project.title,
        ...
        if chapter_context.continuation_point:
            ...
            template = await PromptService.get_template(
                "CHAPTER_GENERATION_ONE_TO_MANY_NEXT", user_id, db_session
            )
            base_prompt = PromptService.format_prompt(
                template,
                project_title=project.title,
            ...
            # 第一章，使用无前置内容模板
            template = await PromptService.get_template(
                "CHAPTER_GENERATION_ONE_TO_MANY", user_id, db_session
            )
            base_prompt = PromptService.format_prompt(
                template,
                project_title=project.title,
...
确保在整个章节创作过程中始终保持风格的一致性。"""
        logger.info(
            f"✅ 批量生成 - 已将写作风格注入系统提示词（{len(style_content)}字符）"
        )
    ...
    try:
        ...
        if plant_result.get("planted_count", 0) > 0:
            logger.info(
                f"🔮 批量生成 - 自动标记伏笔已埋入: {plant_result['planted_count']}个"
            )
    except Exception as plant_error:
        ...
        try:
            ...
            if outline_mode == "one-to-one":
                ...
                if chapter.outline_id:
                    outline_result_temp = await temp_db.execute(
                        select(Outline.structure).where(
                ...
                if outline_structure:
                    try:
                        ...
                        if filter_character_names:
                            logger.info(
                                f"📋 重新生成 - 1-1模式：从structure提取角色列表 {filter_character_names}"
                            )
                    except json.JSONDecodeError:
                        logger.warning(
                            f"⚠️ 重新生成 - outline.structure解析失败，使用全部角色"
                        )
            else:
                ...
                if chapter.expansion_plan:
                    try:
                        ...
                        if filter_character_names:
                            logger.info(
                                f"📋 重新生成 - 1-n模式：从expansion_plan提取角色焦点 {filter_character_names}"
                            )
                    except json.JSONDecodeError:
                        logger.warning(
                            f"⚠️ 重新生成 - expansion_plan解析失败，使用全部角色"
                        )
            ...
            if not style_id:
                ...
                default_style_result = await temp_db.execute(
                    select(ProjectDefaultStyle.style_id).where(
                        ProjectDefaultStyle.project_id == chapter.project_id
                    )
                )
        ...
        try:
            ...
            async for db_session in get_db(request):
                ...
                regen_task = RegenerationTask(
                    ...
                    preserve_elements=regenerate_request.preserve_elements.model_dump()
                ...
                yield await SSEResponse.send_event(
                    event="task_created", data={"task_id": task_id}
                )
                ...
                # === 生成阶段 ===
                full_content = ""
                estimated_total = regenerate_request.target_word_count or len(
                    chapter.content
                )

                yield await tracker.generating(
                    current_chars=0, estimated_total=estimated_total
                )
...
@router.put(
    "/{chapter_id}/expansion-plan", response_model=dict, summary="更新章节规划信息"
)
async def update_chapter_expansion_plan(
    ...
    # 返回更新后的规划数据
    updated_plan = (
        json.loads(chapter.expansion_plan) if chapter.expansion_plan else None
    )
...
async def partial_regenerate_stream(
    ...
    # 验证选中的文本是否匹配
    actual_selected = chapter.content[
        partial_request.start_position : partial_request.end_position
    ]
    if actual_selected != partial_request.selected_text:
        ...
        if partial_request.selected_text in search_area:
            ...
            partial_request.end_position = partial_request.start_position + len(
                partial_request.selected_text
            )
            logger.info(
                f"⚠️ 选中文本位置校正: {partial_request.start_position}-{partial_request.end_position}"
            )
        else:
            raise HTTPException(
                status_code=400, detail="选中的文本与章节内容不匹配，请刷新页面后重试"
            )
    ...
    async def event_generator():
        ...
        try:
            ...
            logger.info(
                f"📝 局部重写 - 原文: {original_word_count}字, 前文: {len(context_before)}字, 后文: {len(context_after)}字"
            )
            ...
            if partial_request.length_mode == "similar":
                ...
                partial_request.length_mode == "custom"
                and partial_request.target_word_count
            ):
                length_requirement = (
                    f"目标字数：约{partial_request.target_word_count}字（允许±20%浮动）"
                )
            else:
                length_requirement = (
                    f"保持与原文相近的字数（约{original_word_count}字）"
                )

            # 获取提示词模板
            template = await PromptService.get_template(
                "PARTIAL_REGENERATE", user_id, db
            )
            if not template:
            ...
            yield await tracker.generating(
                current_chars=0, estimated_total=target_words
            )

            async for chunk in user_ai_service.generate_text_stream(
                prompt=prompt, max_tokens=calculated_max_tokens
            ):
                full_content += chunk
                chunk_count += 1
            ...
            logger.info(
                f"✅ 局部重写完成: 原文{original_word_count}字 -> 新文{new_word_count}字"
            )
...
async def apply_partial_regenerate(
    ...
    if (
        start_position < 0
        or end_position > content_length
    ...
    old_word_count = chapter.word_count or 0
    new_content = (
        chapter.content[:start_position] + new_text + chapter.content[end_position:]
    )
