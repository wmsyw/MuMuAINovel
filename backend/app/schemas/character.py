...
class CharacterBase(BaseModel):
    ...
    role_type: Optional[str] = Field(None, description="角色类型：protagonist/heroine/supporting/antagonist")
...
class CharacterCreate(BaseModel):
    ...
    gender: Optional[str] = Field(None, description="性别")
    is_organization: bool = Field(False, description="是否为组织")
    role_type: Optional[str] = Field("supporting", description="角色类型：protagonist/heroine/supporting/antagonist")
    personality: Optional[str] = Field(None, description="性格特点/组织特性")
    background: Optional[str] = Field(None, description="背景故事")
    appearance: Optional[str] = Field(None, description="外貌特征")
    writing_notes: Optional[str] = Field(None, description="作者私有写作笔记，不注入AI上下文")
