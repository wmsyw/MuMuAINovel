...
USER_ID = "user-inspiration-story-bible"
...
def api_client(monkeypatch: pytest.MonkeyPatch) -> Iterator[ApiHarness]:
    ...
    async def fake_get_user(user_id: str) -> SimpleNamespace:
        return SimpleNamespace(id=user_id, username="story-bible-user", trust_level=1, is_admin=False)
    ...
    async def fake_get_template(template_key: str, user_id: str | None, db: object) -> str:
        ...
        if template_key == "INSPIRATION_STORY_BIBLE":
            return "INSPIRATION_STORY_BIBLE | idea={idea}; card={direction_card_json}; confirmed={confirmed_fields_json}; edits={user_edits_json}; constraints={constraints_json}"
...
def test_generate_story_bible_success_accepts_null_golden_finger_and_does_not_persist(
    api_client: ApiHarness,
) -> None:
    assert api_client.count_rows() == 0
    api_client.fake_ai.queue_json({"story_bible_draft": _story_bible(None), "warnings": []})

    response = api_client.client.post(
        "/api/inspiration/generate-story-bible",
        json={
            "idea": "星桥断裂后的归乡故事",
            "direction_card": _direction_card(None),
            "confirmed_fields": {"target_genre": ["科幻", "冒险"], "core_conflict": "修复星桥"},
            "user_edits": {"tone_and_style": "温暖群像"},
            "constraints": ["不要黑暗结局"],
        },
    )
    ...
    assert set(payload) == {"story_bible_draft", "warnings"}
    assert payload["story_bible_draft"]["golden_finger"] is None
    assert payload["story_bible_draft"]["core_idea"] == "断裂星桥后的归乡故事"
    ...
    assert "温暖群像" in call["system_prompt"]
    assert "不要黑暗结局" in call["system_prompt"]


def test_generate_story_bible_preserves_empty_golden_finger_for_non_fantasy_direction(
    api_client: ApiHarness,
) -> None:
    api_client.fake_ai.queue_json({"story_bible_draft": _story_bible(""), "warnings": ["未使用特殊金手指。"]})
    ...
    response = api_client.client.post(
        ...
        json={
            ...
            "confirmed_fields": {"genre": ["都市", "现实"]},
        },
    )

    assert response.status_code == 200
    payload = response.json()
    assert payload["story_bible_draft"]["golden_finger"] == ""
    assert payload["warnings"] == ["未使用特殊金手指。"]
    assert api_client.count_rows() == 0


def test_generate_story_bible_rejects_malformed_or_missing_required_fields(
    api_client: ApiHarness,
) -> None:
    api_client.fake_ai.queue_text("```json\n{bad json\n```")
    malformed_response = api_client.client.post(
        "/api/inspiration/generate-story-bible",
        json={"idea": "星桥断裂后的归乡故事", "confirmed_fields": {}},
    )
    assert malformed_response.status_code == 422
    malformed_detail = _structured_detail(malformed_response.json())
    assert "JSON" in malformed_detail["message"]

    invalid_story_bible = _story_bible(None)
    invalid_story_bible.pop("core_conflict")
    api_client.fake_ai.queue_json({"story_bible_draft": invalid_story_bible, "warnings": []})
    invalid_response = api_client.client.post(
        "/api/inspiration/generate-story-bible",
        json={"idea": "星桥断裂后的归乡故事", "confirmed_fields": {}},
    )
    ...
    assert api_client.count_rows() == 0
    assert len(api_client.fake_ai.calls) == 2


def test_generate_story_bible_rejects_incomplete_direction_card_before_ai_call(
    api_client: ApiHarness,
) -> None:
    incomplete_card = _direction_card(None)
    incomplete_card.pop("core_conflict")

    response = api_client.client.post(
        "/api/inspiration/generate-story-bible",
        json={
            "idea": "星桥断裂后的归乡故事",
            "direction_card": incomplete_card,
            "confirmed_fields": {},
        },
    )
    ...
    assert api_client.fake_ai.calls == []
    assert api_client.count_rows() == 0


def test_evaluate_story_bible_returns_quality_report_without_persistence(api_client: ApiHarness) -> None:
    ...
    assert payload["dimensions"]["novelty"] == 82
    assert payload["issues"][0]["message"] == "第一章能力展示还可以更明确。"
    assert api_client.count_rows() == 0
...
def test_evaluate_requires_a_direction_card_or_story_bible(api_client: ApiHarness) -> None:
    response = api_client.client.post(
        "/api/inspiration/evaluate",
        json={"context": {"initial_idea": "星桥断裂后的归乡故事"}},
    )

    assert response.status_code == 400
    detail = _structured_detail(response.json())
    assert "需要提供方向卡片或故事圣经草稿" in detail["message"]
    assert api_client.fake_ai.calls == []
    assert api_client.count_rows() == 0
