...
import { bookshelfCardStyles, bookshelfCardClassName } from '../components/common/CardStyles';
...
export default function BookshelfPage({
  ...
  const getRibbonStatusIcon = (displayStatus: string, isWizardIncomplete: boolean, isCompleted: boolean) => {
    ...
    if (isWizardIncomplete) {
      return <LoadingOutlined spin className={sx(commonStyle)} />;
    }
    if (isCompleted) {
      return <CheckCircleOutlined className={sx(commonStyle)} />;
    }
    if (displayStatus.includes('暂停') || displayStatus.includes('搁置')) {
      return <PauseCircleOutlined className={sx(commonStyle)} />;
    }
    if (displayStatus.includes('筹备') || displayStatus.includes('准备') || displayStatus.includes('大纲')) {
      return <BulbOutlined className={sx(commonStyle)} />;
    }

    return <EditOutlined className={sx(commonStyle)} />;
  };
  ...
  return (
    <div>
      ...
      >
        <div className={sx({ position: 'absolute', top: -44, right: -44, width: 150, height: 150, borderRadius: '50%', background: token.colorWhite, opacity: 0.1, pointerEvents: 'none' })} />
        <div className={sx({ position: 'absolute', bottom: -36, left: '26%', width: 100, height: 100, borderRadius: '50%', background: token.colorWhite, opacity: 0.06, pointerEvents: 'none' })} />
        ...
        >
          <div>
            ...
            >
              <BookOutlined className="u-o4kek8" />
              我的书架
            </div>
            <div
              className={sx({
        ...
        <Alert
          ...
          description={
            <div className={sx({
              ...
              <span className={sx({ fontSize: isMobile ? 12 : 14 })}>
      ...
      <Spin spinning={loading}>
        <div className={sx({
          ...
          <div className={sx({ position: 'relative', width: '100%', minWidth: 0, minHeight: isMobile ? mobileBookHeight : desktopBookHeight })}>
            <Card
              hoverable
              className={sx(bookshelfCardClassName, { ...bookshelfCardStyles.newProjectCard, minHeight: isMobile ? mobileBookHeight : desktopBookHeight })}
            ...
            >
              ...
              <div className={sx({
                ...
                <div className={sx({
                  ...
                  <BookOutlined className={sx({ fontSize: isMobile ? 24 : 28 })} />
                </div>

                <div className={sx({ width: '100%', maxWidth: 240, display: 'flex', flexDirection: 'column', gap: isMobile ? 10 : 14 })}>
                  <Button
                    type="primary"
          ...
          {Array.isArray(projects) && projects.map((project, index) => {
            ...
            return (
              <div key={project.id} className={sx({ position: 'relative', width: '100%', minWidth: 0, height: isMobile ? mobileBookHeight : desktopBookHeight })}>
                <Card
                  hoverable
                  className={sx(bookshelfCardClassName, { ...bookshelfCardStyles.projectCard, height: isMobile ? mobileBookHeight : desktopBookHeight })}
                  ...
                  <div className={sx({
                    ...
                    {showCoverFace ? (
                      <div className="u-11zcn0c">
                        ...
                        <div className={sx({ flex: 1, minHeight: isMobile ? 54 : 72 })} />

                        <div className="u-1dqgkrb">
                          <div className={sx({
                            ...
                            <div className={sx({
                              ...
                              <span>完成进度</span>
                              <span className={sx({ color: token.colorWhite, fontWeight: 700 })}>{progress}%</span>
                            </div>
                            <div className="u-ai2232">
                              <div className={sx({
                                width: `${progress}%`,
                                height: '100%',
                                borderRadius: 999,
                          ...
                          <div className={sx({
                            ...
                            <Space size={4} className={sx({ fontSize: isMobile ? 11 : 12, color: 'rgba(255,255,255,0.76)' })}>
                              <CalendarOutlined className={sx({ fontSize: isMobile ? 10 : 12 })} />
                      ...
                      <div className="u-1tv2w9i">
                        {isFlipped && coverGenerating ? (
                          <div className="u-on32qp">
                            <LoadingOutlined spin className={sx({ fontSize: 28, color: token.colorPrimary })} />
                            <div className={sx({ color: token.colorTextSecondary })}>封面生成中，请稍候...</div>
                          </div>
                        ) : isFlipped && coverFailed ? (
                          <div className="u-1b6ccq3">
                            <PictureOutlined className={sx({ fontSize: 36, color: token.colorTextTertiary })} />
                            <div className={sx({ color: token.colorError })}>封面生成失败</div>
                            <div className={sx({ color: token.colorTextSecondary, fontSize: 12 })}>{project.cover_error || '请稍后重试'}</div>
                            <Button
                              type="primary"
                              icon={<ReloadOutlined />}
                              loading={coverActionLoading}
                              onClick={(e) => void handleGenerateCoverClick(e, project, true)}
                              className={sx(coverButtonStyle)}
                            >
                              {coverActionLoading ? '重新生成中...' : '重新生成'}
                            </Button>
                          </div>
                          ...
                          <div className="u-1b6ccq3">
                            <PictureOutlined className={sx({ fontSize: 36, color: token.colorTextTertiary })} />
                            <div className={sx({ color: token.colorTextSecondary })}>当前暂无可用封面</div>
                            <Button
                              type="primary"
                          ...
                          <>
                            <div className={sx({ marginBottom: isMobile ? 10 : 12, paddingRight: isMobile ? 18 : 30 })}>
                              <div className={sx({
                                display: 'flex',
                                alignItems: 'flex-start',
                                gap: 8,
                            ...
                            <div className={sx({ marginBottom: isMobile ? 14 : 18 })}>
                              <div className={sx({
                                display: 'flex',
                                justifyContent: 'space-between',
                                alignItems: 'center',
                                ...
                                <span className={sx({ color: token.colorTextTertiary })}>完成进度</span>
                                <span className={sx({ color: progressColor, fontWeight: 700 })}>{progress}%</span>
                              </div>
                              <div className={sx({
                                height: 6,
                                width: '100%',
                            ...
                            <div className={sx({
                              ...
                              <Space size={4} className={sx({ fontSize: isMobile ? 11 : 12, color: token.colorTextTertiary, justifySelf: 'start' })}>
                                <CalendarOutlined className={sx({ fontSize: isMobile ? 10 : 12 })} />
                                {formatDate(project.updated_at)}
                              </Space>

                              <div className="u-19f2ie">
                                {!coverReady && (
                                  <Button
                                    size="small"
                                    icon={<PictureOutlined />}
                              ...
                              <div className="u-4mg3f2">
                                <Button
                                  type="text"
                                  size="small"
                                  danger
                                  icon={<DeleteOutlined className={sx({ fontSize: isMobile ? 12 : 14 })} />}
                                  onClick={(e) => {
                                    e.stopPropagation();
